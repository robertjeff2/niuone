...
from trading import t_assistant as _t_assistant_feature
...
def _bound_t_assistant_module():
    _t_assistant_feature.bind(sys.modules[__name__])
    return _t_assistant_feature


def is_t_assistant_window(dt: datetime | None = None) -> tuple[bool, str]:
    return _bound_t_assistant_module().is_t_assistant_window(dt)


def _pct(numerator: float, denominator: float) -> float | None:
    return _bound_t_assistant_module()._pct(numerator, denominator)


def _round_lot(value: int | float) -> int:
    return _bound_t_assistant_module()._round_lot(value)


def _price_text(value: Any) -> str:
    return _bound_t_assistant_module()._price_text(value)


def _t_assistant_position_item(
    pos: dict[str, Any],
    code: str,
    *,
    cash: float,
    ...
    return _bound_t_assistant_module()._t_assistant_position_item(
        pos,
        code,
        cash=cash,
...
def evaluate_t_opportunities(state: dict[str, Any], now: datetime | None = None) -> dict[str, Any]:
    return _bound_t_assistant_module().evaluate_t_opportunities(state, now)


def _t_model_text(value: Any, limit: int = 360) -> str:
    return _bound_t_assistant_module()._t_model_text(value, limit)


def normalize_t_model_analysis(raw: dict[str, Any], assessment: dict[str, Any]) -> dict[str, Any]:
    return _bound_t_assistant_module().normalize_t_model_analysis(raw, assessment)


def apply_t_model_analysis(assessment: dict[str, Any], analysis: dict[str, Any]) -> dict[str, Any]:
    return _bound_t_assistant_module().apply_t_model_analysis(assessment, analysis)


def _t_assistant_signature(assessment: dict[str, Any]) -> str:
    return _bound_t_assistant_module()._t_assistant_signature(assessment)


def format_t_assistant_notification(assessment: dict[str, Any]) -> str:
    return _bound_t_assistant_module().format_t_assistant_notification(assessment)
...
def notify_t_assistant_if_needed(
    ...
    *,
    ...
    return _bound_t_assistant_module().notify_t_assistant_if_needed(
        state,
        assessment,
        force=force,
    )
...
def run_t_assistant_once(
    ...
    *,
    ...
    return _bound_t_assistant_module().run_t_assistant_once(
        dt,
        notify=notify,
...
POSITION_MANAGEMENT_DEFAULT = _t_assistant_feature.POSITION_MANAGEMENT_DEFAULT
POSITION_MANAGEMENT_T_ASSISTANT = _t_assistant_feature.POSITION_MANAGEMENT_T_ASSISTANT


def normalize_position_management_mode(
    value: Any,
    *,
    default: str = POSITION_MANAGEMENT_DEFAULT,
) -> str:
    return _t_assistant_feature.normalize_position_management_mode(value, default=default)


def position_management_mode(pos: dict[str, Any] | None) -> str:
    return _t_assistant_feature.position_management_mode(pos)


def is_t_assistant_managed(pos: dict[str, Any] | None) -> bool:
    return _t_assistant_feature.is_t_assistant_managed(pos)
...
def _compact_previous_t_model_analysis(previous: dict[str, Any]) -> dict[str, Any]:
    return _bound_t_assistant_module()._compact_previous_t_model_analysis(previous)
...
def call_model_t_assistant_analysis(
    ...
    return _bound_t_assistant_module().call_model_t_assistant_analysis(
        assessment,
        state,
        previous,
    )
